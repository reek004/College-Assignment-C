...
int main(){
     ...
     for(i=1;i<=n;i+=2){ //the denominator increases by two
        c++; //counter counts the loop each time it runs 
        for(j=1;j<=i;j++){ //loop for getting the factorial value
            f=f*j;
        }
        if(c%2==0){ //if counter is even then its the second term or the even term which is positive
            sum= sum+(1.0/f);
        }
        else{
